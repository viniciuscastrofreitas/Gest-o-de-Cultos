...
const Label = ({ children }: { children?: React.ReactNode }) => (
  <label className="text-[10px] md:text-[11px] font-black text-slate-600 uppercase tracking-widest block mb-2 text-center md:text-left">{children}</label>
);

const FormInput = ({ children, className = "" }: { children?: React.ReactNode, className?: string }) => (
  <div className={`bg-slate-50 border border-slate-200 rounded-2xl px-4 py-3 shadow-sm transition-all focus-within:shadow-md focus-within:bg-white focus-within:border-indigo-100 ${className}`}>
    {children}
  </div>
);
  ...
  const suggestions = useMemo(() => {
    ...
    return fullSongList
      ...
      .sort((a, b) => {
        ...
        // First priority: Main collection before CIAS
  ...
  return (
    <>
      {showSuccessModal && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-6">
          ...
          <div className="relative bg-white rounded-[3.5rem] p-10 w-full max-w-sm shadow-2xl animate-scaleUp text-center border border-white">
            ...
            <p className="text-slate-600 font-bold text-[10px] uppercase tracking-[0.2em] mb-8">Dados registrados com sucesso.</p>
            <button 
      ...
      <div className="max-w-xl mx-auto space-y-6 animate-fadeIn pb-20">
        <div className="bg-white rounded-[3rem] shadow-2xl p-6 md:p-10 border border-white">
          <div className="space-y-6">
              ...
              <div className="flex flex-col items-center">
                ...
                <FormInput className="w-full">
                  <input 
                    ...
                    className="w-full bg-transparent font-black text-lg text-[#1a1c3d] text-center outline-none" 
                  />
                </FormInput>
              ...
              {dateInfo.isSunday && (
                <div className="flex flex-col">
                  ...
                  <div className="flex bg-slate-50 border border-slate-200 p-1 rounded-2xl">
                    <button onClick={() => setDraft(prev => ({ ...prev, description: 'EBD' }))} className={`flex-1 py-2 text-[11px] font-black rounded-xl transition-all ${draft.description === 'EBD' ? 'bg-[#1a1c3d] text-white shadow-lg' : 'text-slate-500'}`}>EBD</button>
                    <button onClick={() => setDraft(prev => ({ ...prev, description: 'DOM' }))} className={`flex-1 py-2 text-[11px] font-black rounded-xl transition-all ${draft.description === 'DOM' ? 'bg-[#1a1c3d] text-white shadow-lg' : 'text-slate-500'}`}>DOM</button>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="space-y-3 pt-4 border-t border-slate-100">
              ...
              <div className="flex gap-2 relative">
                <input 
                  ...
                  className="flex-1 px-5 py-3 bg-slate-50 border border-slate-200 rounded-2xl font-bold text-sm text-[#1a1c3d] outline-none focus:ring-4 focus:ring-indigo-50 focus:bg-white transition-all shadow-sm" 
                ...
                {showSuggestions && suggestions.length > 0 && (
                  <div className="absolute z-[250] top-full left-0 w-full bg-white shadow-2xl rounded-2xl overflow-hidden border border-slate-200 max-h-60 overflow-y-auto mt-1 animate-fadeIn">
                    {suggestions.map(s => (
                      <div key={s} onMouseDown={() => checkAndAddSong(s)} className="p-4 hover:bg-slate-50 cursor-pointer border-b border-slate-50 font-bold text-xs flex justify-between items-center">
                        <span className="text-[#1a1c3d] uppercase">{s}</span>
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center px-1">
                  <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Hinos Selecionados</span>
                ...
                <div className="space-y-2 max-h-60 overflow-y-auto pr-1 custom-scrollbar">
                  {draft.songs.map((s, i) => (
                    <div 
                      ...
                      className={`flex items-center justify-between p-3 bg-slate-50 rounded-xl border border-slate-200 cursor-move transition-all active:scale-95 hover:bg-white hover:shadow-md ${draggedIndex === i ? 'opacity-50' : ''}`}
                    >
                      <div className="flex items-center gap-3 overflow-hidden">
                        <span className="material-icons text-slate-400 text-sm">drag_indicator</span>
                        <span className="font-bold text-[#1a1c3d] text-[12px] truncate">#{i+1} {s}</span>
                      </div>
                      <button onClick={() => setDraft(prev => ({...prev, songs: prev.songs.filter((_, idx) => idx !== i)}))} className="text-rose-400 hover:text-rose-600 ml-2"><span className="material-icons text-lg">close</span></button>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="space-y-4 pt-4 border-t border-slate-100">
              ...
              {dateInfo.isWednesday && (
                <div className="bg-indigo-50/80 p-3 rounded-2xl border border-indigo-100 flex items-center justify-center gap-3 text-indigo-700">
              ...
              {!dateInfo.isWednesday && (
                <div className="flex flex-col min-h-[60px] justify-end">
                  {draft.roles.word === 'TRANSMISSÃO' ? (
                    <div className="bg-amber-50 p-3 rounded-2xl border border-amber-100 flex items-center justify-center gap-3 text-amber-700 animate-fadeIn">
                      <span className="text-lg">📡</span>
            ...
            <button 
              ...
              className={`w-full py-4 rounded-2xl font-black text-base tracking-widest transition-all active:scale-95 flex items-center justify-center gap-3 shadow-xl ${draft.songs.length > 0 ? 'bg-[#1a1c3d] text-white' : 'bg-slate-200 text-slate-500 cursor-not-allowed shadow-none'}`}
            >
            ...
            {editingId && (
              <button 
                ...
                className="w-full py-2 text-slate-500 font-black uppercase text-[10px] tracking-widest hover:text-rose-600 transition-colors"
              >
                CANCELAR
              </button>
            )}
        ...
        <div className="fixed inset-0 z-[8000] flex flex-col justify-end">
          ...
          <div className="relative bg-white rounded-t-[3rem] p-8 pb-12 shadow-2xl animate-slideUp">
            ...
            <div className="flex flex-col items-center text-center">
              ...
              <p className="text-slate-600 font-bold mb-8 leading-relaxed text-[11px]">Cantado há apenas <span className="text-amber-600 font-black">{pendingSong.diff} dias</span>. Deseja prosseguir?</p>
              ...
              <div className="w-full flex flex-col gap-3">
                ...
                <button onClick={() => setPendingSong(null)} className="w-full py-4 text-slate-500 font-black uppercase text-[10px] tracking-widest">CANCELAR</button>
              </div>
            </div>
          </div>
        </div>
